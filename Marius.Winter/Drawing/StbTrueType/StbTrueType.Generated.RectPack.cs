...
internal unsafe partial class StbTrueType
{
		...
		stbtt_pack_range* ranges, int num_ranges)
	{
		...
		for (var j = 0; j < ranges[i].num_chars; ++j)
		...
		var rects = (stbrp_rect*)CRuntime.malloc((ulong)(sizeof(stbrp_rect) * n));
		if (rects == null)
			return 0;
		...
		var return_value = stbtt_PackFontRangesRenderIntoRects(spc, info, ranges, num_ranges, rects);
		CRuntime.free(rects);
		return return_value;
	}
		...
		stbtt_pack_range* ranges, int num_ranges, stbrp_rect* rects)
	{
		...
		for (var i = 0; i < num_ranges; ++i)
		{
			...
			var recip_h = 1.0f / spc.h_oversample;
			var recip_v = 1.0f / spc.v_oversample;
			var sub_x = stbtt__oversample_shift((int)spc.h_oversample);
			var sub_y = stbtt__oversample_shift((int)spc.v_oversample);
			for (var j = 0; j < ranges[i].num_chars; ++j)
			{
				var r = &rects[k];
