...
internal unsafe partial class StbTrueType
{
	...
	public static void stbtt__add_point(UnsafeSpan<stbtt__point> points, int n, float x, float y)
	{
		if (points.IsNull)
			return;
		points[n].x = x;
	...
	public static int stbtt__CompareUTF8toUTF16_bigendian_prefix(byte* s1, int len1, byte* s2, int len2)
	{
		...
		while (len2 != 0)
		{
			...
			else if (ch >= 0xd800 && ch < 0xdc00)
			{
				...
				if (i + 3 >= len1)
					return -1;
				var c = (uint)(((ch - 0xd800) << 10) + (ch2 - 0xdc00) + 0x10000);
				if (s1[i++] != 0xf0 + (c >> 18))
					return -1;
	...
	public static int stbtt__compute_crossings_x(float x, float y, int nverts, UnsafeSpan<stbtt_vertex> verts)
	{
		var orig = stackalloc float[2];
		var ray = stackalloc float[] { 1, 0 };
		var winding = 0;
		var y_frac = (float)CRuntime.fmod(y, 1.0f);
		if (y_frac < 0.01f)
			y += 0.01f;
	...
	public static int stbtt__matches(byte* fc, uint offset, byte* name, int flags)
	{
		...
		if (flags != 0)
		{
			var hd = stbtt__find_table(fc, offset, "head");
	...
	public static void stbtt__tesselate_cubic(UnsafeSpan<stbtt__point> points, ref int num_points, float x0, float y0, float x1,
		float y1, float x2, float y2, float x3, float y3, float objspace_flatness_squared, int n)
	{
		...
		if (flatness_squared > objspace_flatness_squared)
		{
			...
			var mx = (xa + xb) / 2;
			var my = (ya + yb) / 2;
			stbtt__tesselate_cubic(points, ref num_points, x0, y0, x01, y01, xa, ya, mx, my, objspace_flatness_squared,
				n + 1);
			stbtt__tesselate_cubic(points, ref num_points, mx, my, xb, yb, x23, y23, x3, y3, objspace_flatness_squared,
				n + 1);
		}
		else
		{
	...
	public static int stbtt__tesselate_curve(UnsafeSpan<stbtt__point> points, ref int num_points, float x0, float y0, float x1,
		...
		int pw, int ph, int first_char, int num_chars, stbtt_bakedchar* chardata)
	{
		...
		var x = y = 1;
		var bottom_y = 1;
		var scale = stbtt_ScaleForPixelHeight(f, pixel_height);
	...
	public static UnsafeSpan<stbtt__point> stbtt_FlattenCurves(UnsafeSpan<stbtt_vertex> vertices, int num_verts, float objspace_flatness,
		out UnsafeSpan<int> contour_lengths, out int num_contours, void* userdata)
	{
		UnsafeSpan<stbtt__point> points = null;
		var num_points = 0;
		...
		float* descent, float* lineGap)
	{
		...
		var scale = size > 0 ? stbtt_ScaleForPixelHeight(info, size) : stbtt_ScaleForMappingEmToPixels(info, -size);
	...
	public static int ttLONG(byte* p)
	{
		return BitConverter.IsLittleEndian ?
			BinaryPrimitives.ReverseEndianness(Unsafe.ReadUnaligned<int>(p)) :
			Unsafe.ReadUnaligned<int>(p);
	}
	...
	{
		return BitConverter.IsLittleEndian ?
			BinaryPrimitives.ReverseEndianness(Unsafe.ReadUnaligned<short>(p)) :
			Unsafe.ReadUnaligned<short>(p);
	}
	...
	{
		return BitConverter.IsLittleEndian ?
			BinaryPrimitives.ReverseEndianness(Unsafe.ReadUnaligned<uint>(p)) :
			Unsafe.ReadUnaligned<uint>(p);
	}
	...
	{
		return BitConverter.IsLittleEndian ?
			BinaryPrimitives.ReverseEndianness(Unsafe.ReadUnaligned<ushort>(p)) :
			Unsafe.ReadUnaligned<ushort>(p);
	}
