...
internal unsafe partial class StbTrueType
{
		...
		while (e != null)
		{
			...
			else
			{
				...
				if (x_top >= 0 && x_bottom >= 0 && x_top < len && x_bottom < len)
				{
					...
					{
						var x = (int)x_top;
						var height = (sy1 - sy0) * e->direction;
					...
					else
					{
						...
						var x1 = (int)x_top;
						var x2 = (int)x_bottom;
						var y_crossing = y_top + dy * (x1 + 1 - x0);
						var y_final = y_top + dy * (x2 - x0);
	...
	public static void stbtt__rasterize(ref stbtt__bitmap result, UnsafeSpan<stbtt__point> pts, UnsafeSpan<int> wcount, int windings,
		...
		void* userdata, bool useOldRasterizer)
	{
		...
		var e = (stbtt__edge*)CRuntime.malloc((ulong)(sizeof(stbtt__edge) * (n + 1)));
		if (e == null)
			return;
		n = 0;
		var m = 0;
		...
		{
			stbtt__rasterize_sorted_edges_old_rasterizer(ref result, e, n, vsubsample, off_x, off_y, userdata);
		}

		CRuntime.free(e);
	}

	public static void stbtt__rasterize_sorted_edges(ref stbtt__bitmap result, stbtt__edge* e, int n, int vsubsample,
		int off_x, int off_y, void* userdata)
	{
		...
		var scanline2 = scanline + result.w;
		var y = off_y;
	...
	public static void stbtt__sort_edges_quicksort(stbtt__edge* p, int n)
	{
		while (n > 12)
		{
			...
			var m = n >> 1;
			var c01 = (&p[0])->y0 < (&p[m])->y0 ? 1 : 0;
			var c12 = (&p[m])->y0 < (&p[n - 1])->y0 ? 1 : 0;
			if (c01 != c12)
			{
				var c = (&p[0])->y0 < (&p[n - 1])->y0 ? 1 : 0;
				var z = c == c12 ? 0 : n - 1;
				t = p[z];
				p[z] = p[m];
				p[m] = t;
	...
	public static void stbtt_Rasterize(ref stbtt__bitmap result, float flatness_in_pixels, UnsafeSpan<stbtt_vertex> vertices,
		int num_verts, float scale_x, float scale_y, float shift_x, float shift_y, int x_off, int y_off, int invert,
		void* userdata, bool useOldRasterizer)
	{
		var scale = scale_x > scale_y ? scale_y : scale_x;
		var windings = stbtt_FlattenCurves(vertices, num_verts, flatness_in_pixels / scale, out var winding_lengths, out var winding_count, userdata);
		if (!windings.IsNull)
		{
			stbtt__rasterize(ref result, windings, winding_lengths, winding_count, scale_x, scale_y, shift_x, shift_y,
