...
internal unsafe partial class StbImage
{
		...
		stbi__result_info* ri)
	{
		...
		if (stbi__bmp_parse_header(s, &info) == null)
			return null;
		var flip_vertically = (int)s.img_y > 0 ? 1 : 0;
		...
		if (stbi__mad3sizes_valid(target, (int)s.img_x, (int)s.img_y, 0) == 0)
			return (byte*)(ulong)(stbi__err("too large") != 0 ? 0 : 0);
		var _out_ = (byte*)stbi__malloc_mad3(target, (int)s.img_x, (int)s.img_y, 0);
		if (_out_ == null)
			...
			for (j = 0; j < (int)s.img_y; ++j)
			{
				if (easy != 0)
				{
					for (i = 0; i < (int)s.img_x; ++i)
					{
						...
						z += 3;
						var a = (byte)(easy == 2 ? stbi__get8(s) : 255);
						all_a |= a;
						if (target == 4)
							_out_[z++] = a;
					}
				...
				else
				{
					...
					for (i = 0; i < (int)s.img_x; ++i)
					{
						...
						var a = (uint)(ma != 0 ? stbi__shiftsigned(v & ma, ashift, acount) : 255);
						all_a |= a;
						if (target == 4)
							_out_[z++] = (byte)(a & 255);
					}
	...
	public static int stbi__bmp_info(stbi__context s, int* x, int* y, int* comp)
	{
		var info = new stbi__bmp_data();
		info.all_a = 255;
		var p = stbi__bmp_parse_header(s, &info);
		if (p == null)
		{
			stbi__rewind(s);
			return 0;
	...
	public static int stbi__bmp_test_raw(stbi__context s)
	{
		...
		stbi__get16le(s);
		stbi__get32le(s);
		var sz = (int)stbi__get32le(s);
		var r = sz == 12 || sz == 40 || sz == 56 || sz == 108 || sz == 124 ? 1 : 0;
		return r;
	}
