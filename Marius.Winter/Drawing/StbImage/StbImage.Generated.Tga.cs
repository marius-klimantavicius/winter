...
internal unsafe partial class StbImage
{
	public static int stbi__tga_test(stbi__context s)
	{
		var res = 0;
		stbi__get8(s);
		var tga_color_type = stbi__get8(s);
		if (tga_color_type > 1)
			goto errorEnd;
		var sz = stbi__get8(s);
		if (tga_color_type == 1)
		...
		stbi__result_info* ri)
	{
		...
		*y = tga_height;
		...
		if (stbi__mad3sizes_valid(tga_width, tga_height, tga_comp, 0) == 0)
			return (byte*)(ulong)(stbi__err("too large") != 0 ? 0 : 0);
		var tga_data = (byte*)stbi__malloc_mad3(tga_width, tga_height, tga_comp, 0);
		if (tga_data == null)
	...
	public static int stbi__tga_info(stbi__context s, int* x, int* y, int* comp)
	{
		...
		stbi__get8(s);
		var tga_colormap_type = stbi__get8(s);
