...
internal unsafe partial class StbImage
{
	...
	private static ReadOnlySpan<int> InitialXorig => [ 0, 4, 0, 2, 0, 1, 0 ];
	private static ReadOnlySpan<int> InitialYorig => [ 0, 0, 4, 0, 2, 0, 1 ];
	private static ReadOnlySpan<int> InitialXspc => [ 8, 8, 4, 4, 2, 2, 1 ];
	private static ReadOnlySpan<int> InitialYspc => [ 8, 8, 8, 4, 4, 2, 2 ];
		...
		int depth, int color)
	{
		...
		var filter_buf = (byte*)stbi__malloc_mad2((int)img_width_bytes, 2, 0);
		if (filter_buf == null)
			return stbi__err("outofmem");
		if (depth < 8)
		{
		...
		int depth, int color, int interlaced)
	{
		...
		for (var p = 0; p < 7; ++p)
		{
			InitialXorig.CopyTo(new Span<int>(xorig, InitialXorig.Length));
			InitialYorig.CopyTo(new Span<int>(yorig, InitialYorig.Length));
			InitialXspc.CopyTo(new Span<int>(xspc, InitialXspc.Length));
			InitialYspc.CopyTo(new Span<int>(yspc, InitialYspc.Length));
			...
			switch (c.type)
			{
				...
				case ((uint)73 << 24) + ((uint)72 << 16) + ((uint)68 << 8) + 82:
				{
					...
					if (color == 3)
						...
						return stbi__err("bad ctype");
					var comp = stbi__get8(s);
					if (comp != 0)
						return stbi__err("bad comp method");
					var filter = stbi__get8(s);
					if (filter != 0)
				...
				case ((uint)73 << 24) + ((uint)68 << 16) + ((uint)65 << 8) + 84:
				{
					...
					if (ioff + c.length > idata_limit)
					{
						...
						var p = (byte*)CRuntime.realloc(z.idata, (ulong)idata_limit);
						if (p == null)
							return stbi__err("outofmem");
						z.idata = p;
					}
