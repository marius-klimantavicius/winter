...
internal unsafe partial class StbImage
{
	...
	public static int stbi__jpeg_test(stbi__context s)
	{
		...
		j.s = s;
		stbi__setup_jpeg(j);
		var r = stbi__decode_jpeg_header(j, STBI__SCAN_type);
		stbi__rewind(s);
		return r;
	}
		...
		stbi__result_info* ri)
	{
		...
		j.s = s;
		stbi__setup_jpeg(j);
		var result = load_jpeg_image(j, x, y, comp, req_comp);
		return result;
	}
	...
	public static int stbi__jpeg_info(stbi__context s, int* x, int* y, int* comp)
	{
		...
		if (j == null)
			return stbi__err("outofmem");
		j.s = s;
		var result = stbi__jpeg_info_raw(j, x, y, comp);
		return result;
	}
	...
	public static int stbi__jpeg_huff_decode(stbi__jpeg j, stbi__huffman* h)
	{
		...
		var c = (int)((j.code_buffer >> (32 - 9)) & ((1 << 9) - 1));
		var k = h->fast[c];
		if (k < 255)
		{
			int s = h->size[k];
	...
	public static int stbi__extend_receive(stbi__jpeg j, int n)
	{
		...
		if (j.code_bits < n)
			return 0;
		var sgn = (int)(j.code_buffer >> 31);
		var k = CRuntime._lrotl(j.code_buffer, n);
	...
	public static int stbi__parse_entropy_coded_data(stbi__jpeg z)
	{
		...
		if (z.progressive == 0)
		{
			...
			{
				var data = stackalloc short[64];
				for (var j = 0; j < z.img_mcu_y; ++j)
				for (var i = 0; i < z.img_mcu_x; ++i)
				{
					for (var k = 0; k < z.scan_n; ++k)
					{
						var n = z.order[k];
						for (var y = 0; y < z.img_comp[n].v; ++y)
						for (var x = 0; x < z.img_comp[n].h; ++x)
						{
	...
	public static void stbi__YCbCr_to_RGB_row(byte* _out_, byte* y, byte* pcb, byte* pcr, int count, int step)
	{
		for (var i = 0; i < count; ++i)
		{
			...
			var cr = pcr[i] - 128;
			var cb = pcb[i] - 128;
			var r = y_fixed + cr * ((int)(1.40200f * 4096.0f + 0.5f) << 8);
			var g = (int)(y_fixed + cr * -((int)(0.71414f * 4096.0f + 0.5f) << 8) +
				((cb * -((int)(0.34414f * 4096.0f + 0.5f) << 8)) & 0xffff0000));
			var b = y_fixed + cb * ((int)(1.77200f * 4096.0f + 0.5f) << 8);
			r >>= 20;
			g >>= 20;
			b >>= 20;
			if ((uint)r > 255)
	...
	public static byte* load_jpeg_image(stbi__jpeg z, int* out_x, int* out_y, int* comp, int req_comp)
	{
		...
		var n = req_comp != 0 ? req_comp : z.s.img_n >= 3 ? 3 : 1;
		var is_rgb = z.s.img_n == 3 && (z.rgb == 3 || (z.app14_color_transform == 0 && z.jfif == 0)) ? 1 : 0;
...

		{
			...
			var output = (byte*)stbi__malloc_mad3(n, (int)z.s.img_x, (int)z.s.img_y, 1);
			if (output == null)
			{
				stbi__cleanup_jpeg(z);
				return (byte*)(ulong)(stbi__err("outofmem") != 0 ? 0 : 0);
