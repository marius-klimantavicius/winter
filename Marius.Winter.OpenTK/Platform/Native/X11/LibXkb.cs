...
internal static class LibXkb
{
    ...
    [DllImport(Xkb, CallingConvention = CallingConvention.Cdecl)]
    internal static extern unsafe XkbDescRec* XkbGetMap(XDisplayPtr display, uint which, uint device_spec);

    [DllImport(Xkb, CallingConvention = CallingConvention.Cdecl)]
    // FIXME: Double check that int is the correct type for status
    internal static extern unsafe int /* Status */ XkbGetNames(XDisplayPtr display, uint which, XkbDescRec* xkb);
    ...
    [DllImport(Xkb, CallingConvention = CallingConvention.Cdecl)]
    internal static extern unsafe void XkbFreeKeyboard(XkbDescRec* xkb, uint which, bool free_all);
}