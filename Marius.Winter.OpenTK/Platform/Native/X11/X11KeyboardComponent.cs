...
public class X11KeyboardComponent : IKeyboardComponent
{
    ...
    internal static Key TranslateKeySym(ReadOnlySpan<XKeySym> keysyms)
    {
        if (keysyms.Length > 1)
        {
    ...
    private static readonly Dictionary<string, Scancode> XkbNameToScancode = new ()
    {
        { "AE01", Scancode.D1 },
        { "AE02", Scancode.D2 },
        { "AE03", Scancode.D3 },
    ...
    // FIXME: static...
    private static readonly Scancode[] KeycodeToScancode = new Scancode[256];
    private static readonly int[] ScancodeToKeycode = new int[256];
    ...
    internal static uint KeySymToUnicode(XKeySym value)
    {
        var keysym = value.Id;
        ...
        if ((keysym & 0xff000000) == 0x01000000)
            return (uint)(keysym & 0x00ffffff);

        return keysymtab.GetValueOrDefault((uint)keysym, uint.MaxValue);
    }

    private static readonly Dictionary<uint, uint> keysymtab = new Dictionary<uint, uint>
    {
        { 0x01a1, 0x0104 },
        { 0x01a2, 0x02d8 },
        { 0x01a3, 0x0141 },
