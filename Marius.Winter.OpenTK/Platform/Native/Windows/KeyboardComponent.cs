...
public class KeyboardComponent : IKeyboardComponent
{
    ...
    private readonly Key[] Keymap = new Key[256];
    ...
    private string LayoutDisplayNameFromKeyboardLayoutName(string layoutName)
    {
        ...
        var status = Win32.RegOpenKeyEx((UIntPtr)PredefinedKeys.HKEY_LOCAL_MACHINE, LAYOUTS_REGISTRY_PATH, 0, AccessMask.KeyRead, out UIntPtr layoutsKey);
        if (status != 0)
        {
    ...
    private static readonly Scancode[] ScancodeLookup = new Scancode[256]
    {
        // 0x00 - 0x07
    ...
    private static readonly Scancode[] ScancodeLookupExt = new Scancode[256]
    {
        // 0x00 - 0x07
        0, 0, 0, 0, 0, 0, 0, 0,
        // 0x08 - 0x0F
    ...
    private static readonly Key[] KeyLookup = new Key[256]
    {
        // 0x00 - 0x07
