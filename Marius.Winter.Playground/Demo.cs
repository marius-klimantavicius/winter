...
using Texture2D = NvgSharp.OpenTK.OpenGL.Texture;
...
public class Demo
{
	...
	private readonly SpriteFontBase fontSans18;
	private readonly SpriteFontBase fontSans20;
	private readonly SpriteFontBase fontBold18;
	private readonly SpriteFontBase fontBold20;
	private readonly Texture2D[] images = new Texture2D[12];
	...
	public Demo(NvgContext vg)
	{
		...
		{
			var path = "/home/marius/p/NvgSharp/samples/Assets/images/image" + (i + 1).ToString() + ".jpg";

			var texture = Texture.FromFile(path);
			images[i] = texture;
		}

		fontSystemIcons = LoadFont("/home/marius/p/NvgSharp/samples/Assets/entypo.ttf");
		fontSystemNormal = LoadFont("/home/marius/p/NvgSharp/samples/Assets/Roboto-Regular.ttf");
		fontSystemBold = LoadFont("/home/marius/p/NvgSharp/samples/Assets/Roboto-Bold.ttf");
	...
	public void drawWindow(NvgContext vg, string title, float x, float y, float w, float h)
	{
		...
		// Drop shadow
		var shadowPaint = vg.BoxGradient(x, y + 2, w, h, cornerRadius * 2, 10, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
		vg.BeginPath();
		...
		// Header
		var headerPaint = vg.LinearGradient(x, y, x, y + 15, Utility.FromRGBA(255, 255, 255, 8), Utility.FromRGBA(0, 0, 0, 16));
		vg.BeginPath();
	...
	public void drawSearchBox(NvgContext vg, string text, float x, float y, float w, float h)
	{
		...
		// Edit
		var bg = vg.BoxGradient(x, y + 1.5f, w, h, h / 2, 5, Utility.FromRGBA(0, 0, 0, 16), Utility.FromRGBA(0, 0, 0, 92));
		vg.BeginPath();
		vg.RoundedRect(x, y, w, h, cornerRadius);
		vg.FillPaint(bg);
		vg.Fill();
	...
	public void drawDropDown(NvgContext vg, string text, float x, float y, float w, float h)
	{
		...
		var bg = vg.LinearGradient(x, y, x, y + h, Utility.FromRGBA(255, 255, 255, 16), Utility.FromRGBA(0, 0, 0, 16));
		vg.BeginPath();
	...
	public static void drawEditBoxBase(NvgContext vg, float x, float y, float w, float h)
	{
		var bg =
			// Edit
			vg.BoxGradient(x + 1, y + 1 + 1.5f, w - 2, h - 2, 3, 4, Utility.FromRGBA(255, 255, 255, 32), Utility.FromRGBA(32, 32, 32, 32));
		vg.BeginPath();
	...
	public void drawCheckBox(NvgContext vg, string text, float x, float y, float w, float h)
	{
		...
		var bg = vg.BoxGradient(x + 1, y + (int)(h * 0.5f) - 9 + 1, 18, 18, 3, 3, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 92));
		vg.BeginPath();
	...
	public void drawButton(NvgContext vg, string preicon, string text, float x, float y, float w, float h, Color col)
	{
		...
		var bg = vg.LinearGradient(x, y, x, y + h, Utility.FromRGBA(255, 255, 255, isBlack(col) ? (byte)16 : (byte)32), 
	...
	public static void drawSlider(NvgContext vg, float pos, float x, float y, float w, float h)
	{
		...
		//	ClearState(vg);
		// Slot
		var bg = vg.BoxGradient(x, cy - 2 + 1, w, 4, 2, 2, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 128));
		vg.BeginPath();
		...
		// Knob
		var knob = vg.LinearGradient(x, cy - kr, x, cy + kr, Utility.FromRGBA(255, 255, 255, 16), Utility.FromRGBA(0, 0, 0, 16));
		vg.BeginPath();
	...
	public static void drawEyes(NvgContext vg, float x, float y, float w, float h, float mx, float my, float t)
	{
		...
		var blink = 1 - (float)(Math.Pow(Math.Sin(t * 0.5f), 200) * 0.8f);

		var bg = vg.LinearGradient(x, y + h * 0.5f, x + w * 0.1f, y + h, Utility.FromRGBA(0, 0, 0, 32), Utility.FromRGBA(0, 0, 0, 16));
		vg.BeginPath();
		...
		var gloss = vg.RadialGradient(lx - ex * 0.25f, ly - ey * 0.5f, ex * 0.1f, ex * 0.75f, Utility.FromRGBA(255, 255, 255, 128), Utility.FromRGBA(255, 255, 255, 0));
		vg.BeginPath();
		vg.Ellipse(lx, ly, ex, ey);
		vg.FillPaint(gloss);
		vg.Fill();
	...
	public static void drawGraph(NvgContext vg, float x, float y, float w, float h, float t)
	{
		...
		// Graph background
		var bg = vg.LinearGradient(x, y, x, y + h, Utility.FromRGBA(0, 160, 192, 0), Utility.FromRGBA(0, 160, 192, 64));
		vg.BeginPath();
		vg.MoveTo(sx[0], sy[0]);
		for (i = 1; i < 6; i++)
	...
	public static void drawSpinner(NvgContext vg, float cx, float cy, float r, float t)
	{
		...
		vg.ClosePath();
		var ax = cx + (float)Math.Cos(a0) * (r0 + r1) * 0.5f;
		var ay = cy + (float)Math.Sin(a0) * (r0 + r1) * 0.5f;
		var bx = cx + (float)Math.Cos(a1) * (r0 + r1) * 0.5f;
		var by = cy + (float)Math.Sin(a1) * (r0 + r1) * 0.5f;
		var paint = vg.LinearGradient(ax, ay, bx, by, Utility.FromRGBA(0, 0, 0, 0), Utility.FromRGBA(0, 0, 0, 128));
		vg.FillPaint(paint);
		vg.Fill();
	...
	public static void drawThumbnails(NvgContext vg, float x, float y, float w, float h, Texture2D[] images, float t)
	{
		...
		var thumb = 60.0f;
		var arry = 30.5f;
		var stackh = (images.Length / 2) * (thumb + 10) + 10;
		var u = (1 + (float)Math.Cos(t * 0.5f)) * 0.5f;
		var u2 = (1 - (float)Math.Cos(t * 0.2f)) * 0.5f;

		vg.SaveState();
		//	ClearState(vg);
		// Drop shadow
		var shadowPaint = vg.BoxGradient(x, y + 4, w, h, cornerRadius * 2, 20, Utility.FromRGBA(0, 0, 0, 128), Utility.FromRGBA(0, 0, 0, 0));
		vg.BeginPath();
		vg.Rect(x - 10, y - 10, w + 20, h + 30);
		vg.RoundedRect(x, y, w, h, cornerRadius);
		vg.PathWinding(Solidity.Hole);
		...
		for (var i = 0; i < images.Length; i++)
		{
			...
			var v = i * dv;
			var a = clampf((u2 - v) / dv, 0, 1);

			if (a < 1.0f)
				drawSpinner(vg, tx + thumb / 2, ty + thumb / 2, thumb * 0.25f, t);

			var imgPaint = vg.ImagePattern(tx + ix, ty + iy, iw, ih, 0.0f / 180.0f * (float)Math.PI, images[i], a);
			vg.BeginPath();
		...
		// Hide fades
		var fadePaint = vg.LinearGradient(x, y, x, y + 6, Utility.FromRGBA(200, 200, 200, 255), Utility.FromRGBA(200, 200, 200, 0));
		vg.BeginPath();
		vg.Rect(x + 4, y, w - 8, 6);
	...
	public static void drawColorwheel(NvgContext vg, float x, float y, float w, float h, float t)
	{
		...
		for (var i = 0; i < 6; i++)
		{
			var a0 = (float)i / 6.0f * (float)Math.PI * 2.0f - aeps;
			var a1 = (float)(i + 1.0f) / 6.0f * (float)Math.PI * 2.0f + aeps;
			vg.BeginPath();
			vg.Arc(cx, cy, r0, a0, a1, Winding.ClockWise);
			vg.Arc(cx, cy, r1, a1, a0, Winding.CounterClockWise);
			vg.ClosePath();
	...
	{
		var caps = new[] { LineCap.Butt, LineCap.Round, LineCap.Square };
		var lineWidth = 8.0f;
