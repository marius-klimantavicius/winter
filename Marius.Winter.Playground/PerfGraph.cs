...
public class PerfGraph
{
	...
	private readonly float[] _values = new float[100];
	private int _head;
	...
	public void Render(NvgContext vg, float x, float y)
	{
		...
		if (_style == Style.GRAPH_RENDER_FPS)
		{
			...
			{
				var v = 1.0f / (0.00001f + _values[(_head + i) % _values.Length]);
				if (v > 80.0f)
					v = 80.0f;
				var vx = x + ((float)i / (_values.Length - 1)) * w;
				var vy = y + h - ((v / 80.0f) * h);
				vg.LineTo(vx, vy);
			}
		}
		else if (_style == Style.GRAPH_RENDER_PERCENT)
		{
			for (i = 0; i < _values.Length; i++)
			{
				var v = _values[(_head + i) % _values.Length] * 1.0f;
				if (v > 100.0f)
					v = 100.0f;
				var vx = x + ((float)i / (_values.Length - 1)) * w;
		...
		else
		{
			for (i = 0; i < _values.Length; i++)
			{
				var v = _values[(_head + i) % _values.Length] * 1000.0f;
				if (v > 20.0f)
					v = 20.0f;
				var vx = x + ((float)i / (_values.Length - 1)) * w;
